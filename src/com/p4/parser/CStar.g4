...
dcl: TYPE (ID | assign | array_assign) SEMICOLON; //done
assign: ID ASSIGN_OP expr; //done
...
factor:	ID | val | array_value | LEFT_PAREN expr RIGHT_PAREN; //done

array_assign: ARRAY ID ASSIGN_OP array_expr; //done
array_expr: LEFT_BRACKET val (COMMA val)* RIGHT_BRACKET; //done
array_value: ID LEFT_BRACKET val RIGHT_BRACKET (ASSIGN_OP val)?; //done
...
func_call: (ID DOT)? ID LEFT_PAREN ((ID | val) (COMMA (ID | val))*)? RIGHT_PAREN; //done
